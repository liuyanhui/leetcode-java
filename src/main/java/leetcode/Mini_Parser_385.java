...
public class Mini_Parser_385 {
     ...
     * review
     * round 2
     * 使用stack时，不要提前入栈，需要的才操作并入栈。
     * 如果提前入栈，需要考虑多个[num,num,num]的情况。反而会复杂。
     * TIP此类问题的套路：把数字计算和对象处理分离，一段逻辑只负责相对单一的功能。可以假定数字单字符数字，进而简化问题的复杂度。
     *
     * 非digit类型的字符要么表示一个对象逻辑的开始，要么表示一个对象逻辑的结束。而digit类型的字符，只负责计算逻辑。
     * 当前字符不是digit，前一个字符是digit时，需要先额外处理num部分，再处理当前字符应该处理的逻辑。
     * 当前字符不是digit，前一个字符是digit时，处理当前字符应该处理的逻辑，无需考虑num。
     */
